...
export const getProjects = async (chainId) => {
    const query = `{
        ...
        if (res.isSuccess) {
            ...
            projectsList = projectsList.map((project) => {
                ...
                if (qfRound && qfRound.id == project.qfRoundID) {
                    ...
                    return { ...project, index: index++, isFinished: isFinished, isOnQF: isOnQF, matchingPool: qfRound.amount, qfRaised: project.qfMatched / qfRound.totalRootSum * qfRound.amount }
                }

                return { ...project, index: index++, isFinished: isFinished, isOnQF: false, matchingPool: 0, qfRaised: 0 }
            }
            )
